...
def get_sum_nodes_values(node):
    if not node:
        return 0  
    return node.key + get_sum_nodes_values(node.left) + get_sum_nodes_values(node.right)  # Рекурсивно обчислюємо суму
...
print("Мінімальне значення у дереві:", min_value_node(root).key)
print("Максимальне значення у дереві:", max_value_node(root).key)
